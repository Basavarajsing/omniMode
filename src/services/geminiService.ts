...
export const analyzeEmotion = async (
    // FIX: Removed apiKey from parameters as it must be handled by environment variables per guidelines.
    ...
    try {
        ...
        if (!process.env.API_KEY) {
            throw new Error("API_KEY is not provided in environment variables.");
        }
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

        const prompt = getPrompt(text);
        // FIX: Initialize `parts` with a type that allows adding both text and inlineData parts to resolve the type error.
        ...
        // FIX: Updated error messages to be more appropriate since the user cannot change the API key.
        if (error instanceof Error && error.message.includes('API key not valid')) {
             throw new Error("API key is not valid. Please contact the application administrator.");
        }
...
export const analyzeVideoFrames = async (base64Frames: string[]): Promise<EmotionResult[]> => {
    ...
    for (let i = 0; i < base64Frames.length; i++) {
        try {
            // FIX: Removed apiKey from the call as it's now handled within analyzeEmotion.
            ...
            if (frameResult) {
             ...
             // Re-throw the error to be caught by the main handler in App.tsx
             if (error instanceof Error && error.message.includes('API key not valid')) {
                throw error;
             }
